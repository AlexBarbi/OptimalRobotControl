...
def define_terminal_cost_and_constraints(opti, X, S, c_path, r_path, w_final, w_p):
    # Compute the end-effector position using forward kinematics
    ee_pos = fk(X[-1][:nq])
...
def create_and_solve_ocp(N, nx, nq, lbx, ubx, dt, x_init,
    ...
    running_cost = define_running_cost_and_dynamics(opti, X, U, S, W, N, dt, x_init,
                                                    c_path, r_path, w_v, w_a, w_p,
                                                    tau_min, tau_max)
    terminal_cost = define_terminal_cost_and_constraints(opti, X, S, c_path, r_path, w_final, w_p)
...
if __name__ == "__main__":
    ...
    log_w_v, log_w_a, log_w_w = -3, -3, -2
    log_w_final = 0
    log_w_p = 2 

    sol, X, U, S, W = create_and_solve_ocp(
        N, nx, nq, lbx, ubx, dt, x_init, c_path, r_path,
        10**log_w_v, 10**log_w_a, 10**log_w_final, 10**log_w_p,
        tau_min, tau_max
    )
